...
C $Header: /project/work/rep/STENEX/src/se_snl/swap_sandia_routines.f,v 1.2 2004/03/26 16:25:25 yoj Exp $
...
c inter-processor swap within a 1d array
      ...
      use swap_sandia   ! inherits se_pe_info_ext
      implicit none
      include 'mpif.h'
      ...
      if ( recvproc >= 0 ) then
         ...
         call MPI_Irecv( rbuf, n1, MPI_REAL, recvproc, 0, se_worker_comm,
     &                   irequest, ierror )
      ...
      if ( sendproc >= 0 ) then
         ...
         call MPI_Send( sbuf, n1, MPI_REAL, sendproc, 0, se_worker_comm,
     &                  ierror )
...
c inter-processor swap within a 2d array
      ...
      if ( recvproc >= 0 ) then
         ...
         call MPI_Irecv( rbuf, ntotal, MPI_REAL, recvproc, 0, se_worker_comm,
     &                   irequest, ierror )
      end if
      ...
      if ( sendproc >= 0 ) then
         ...
         call MPI_Send( sbuf, ntotal, MPI_REAL, sendproc, 0, se_worker_comm,
     &                  ierror )
