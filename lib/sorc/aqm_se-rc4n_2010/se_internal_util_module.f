...
C $Header: /project/work/rep/STENEX/src/se_f90/se_internal_util_module.f,v 1.3 2001/01/24 13:14:30 yoj Exp $
...
C -- each PE send PA grid index info to PE 0, gathering process
           ...
           call mpi_gather (index(1, 1, se_myworker_pe+1), 4, mpi_integer, 
     &                      index(1, 1, se_myworker_pe+1), 4, mpi_integer, 
     &                      0, se_worker_comm, error)

C -- PE 0 boardcast entire PA grid index info

           call mpi_bcast (index(1, 1, 1), 4*se_numworkers, mpi_integer, 0,
