...
C $Header: /project/air4/yoj/rel/models/PARIO/src/subdmap.f,v 1.1.1.1 2003/05/05 17:18:09 yoj Exp $

      SUBROUTINE SUBDMAP ( NUMPROCS, NROWS, NCOLS, NLAYS,
     &                     NPCOL, NPROW, NROWS_PE, NCOLS_PE, ROWSX_PE,
     &                     COLSX_PE, MAXCELLS, IERR )
C ....................................................................
...
C  OUT:
C        INTEGER    NROWS_PE(NUMPROCS)     ! Number of rows for each PE.
C        INTEGER    NCOLS_PE(NUMPROCS)     ! Number of columns for each PE.
C        INTEGER    ROWSX_PE(2,NUMPROCS)   ! Row index range for each PE.
C        INTEGER    COLSX_PE(2,NUMPROCS)   ! Column index range for each PE.
C        INTEGER    MAXCELLS               ! Maximum subdomain size over PEs.
C        INTEGER    IERR
 
C  LOCAL VARIABLE DESCRIPTION:  see below
 
C  CALLS: ALLOINT, MEMFREE, M3WARN
...
C EXTERNAL FUNCTIONS:
      ...
      EXTERNAL      ALLOINT, MEMFREE            ! Memory allocation library.
