...
C $Header: /project/air4/yoj/rel/models/PARIO/src/pwrite3.f,v 1.1.1.1 2003/05/05 17:18:09 yoj Exp $ 
...
C File written to on previous call to pwrite3.
      ...
      INTEGER        PACKAGE(4)  ! MPI boardcast data

      CHARACTER*16   VNAM16      ! Scratch area for variable name.

      LOGICAL        PWRGRDD          ! Parallel write for gridded data files.
      EXTERNAL       PWRGRDD          ! Parallel M3IO library.
...
C.......  Operation valid only for gridded files.
     ...
     &                 // ' to file '// FNAME( 1:TRIMLEN( FNAME ) )
          ...
          ELSE
             ...
             IF ( FLCHANGE ) THEN
                CALL GET_WRITE_MAP ( NUMPROCS, NPROWD, NPCOLD, GNROWS, GNCOLS,
     &                               NROWS3D, NCOLS3D, NLAYS3D, NROWS_PE,
     &                               NCOLS_PE, ROWSX_PE, COLSX_PE, WR_NROWS_PE,
     &                               WR_NCOLS_PE, WR_ROWSX_PE, WR_COLSX_PE )
             END IF 
...
C.......   Calculate buffer position.
     ...
     &                             BUFFER(BP), NROWS3D, NCOLS3D, 
     &                             NLAYS3D, NUMROWS, NUMCOLS, 
     &                             NUMPROCS, WR_NROWS_PE, WR_NCOLS_PE,
