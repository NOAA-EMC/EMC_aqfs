...
C                System
C File: @(#)$Id: asgnspro.f,v 1.12 2004/06/28 14:31:29 cseppan Exp $
C
...
C smoke@unc.edu
C
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/spcmat/asgnspro.f,v $
C Last updated: $Date: 2004/06/28 14:31:29 $ 
C
C***************************************************************************
...
C                       pollutant-specific PLANT non-blank       match
            ...
            IF( F6 .GT. 0 ) THEN
	      IF (CSPT16(F6,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT16( F6,V )
	      ...
	      ENDIF	
            ...
            ELSEIF( F5 .GT. 0 ) THEN
	       IF ( CSPT15(F5,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT15( F5,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF 
            ELSEIF( F4 .GT. 0 ) THEN
	       IF (CSPT14(F4,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT14( F4,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
	       ENDIF	
            ...
            ELSEIF( F3 .GT. 0 ) THEN
	       IF (CSPT13(F3,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT13( F3,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF
            ELSEIF( F2 .GT. 0 ) THEN 
	       IF (CSPT12(F2,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT12( F2,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF
            ELSEIF( F1 .GT. 0 ) THEN
	       IF (CSPT11(F1,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT11( F1,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF
            ELSEIF( F0 .GT. 0 ) THEN
	       IF (CSPT10(F0,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT10( F0,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF
...
C                       pollutant-specific left SIC match
                ...
                IF( F5 .GT. 0 ) THEN
		  IF(CSPT31( F5,V ) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT31( F5,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
                ELSEIF(F4 .GT. 0 ) THEN
		  IF(CSPT30(F4,V) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT30( F4,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
                ELSEIF(F3 .GT. 0 ) THEN
		  IF(CSPT29(F3,V) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT29( F3,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
                ELSEIF(F2 .GT. 0 ) THEN
		  IF( CSPT28(F2,V) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT28( F2,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
                ELSEIF(F1 .GT. 0 ) THEN
		  IF(CSPT27(F1,V) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT27( F1,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
                ELSEIF(F0 .GT. 0 ) THEN
		  IF( CSPT26(F0,V) .NE. EMCMISS3 ) THEN
                    SPCODE = CSPT26( F0,V )
                    CALL SETSOURCE_SMATS
                    CYCLE                       !  to end of sources-loop
                  ENDIF
...
C              example).
            ...
            IF( F5 .GT. 0 ) THEN
	      IF( CSPT09(F5,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT09( F5,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
              ENDIF
            ELSEIF( F4 .GT. 0 ) THEN
	      IF( CSPT08(F4,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT08( F4,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
              ENDIF 
            ELSEIF( F4B .GT. 0 ) THEN
	      IF( CSPT09(F4B,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT09( F4B,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
              ENDIF
            ELSEIF( F3 .GT. 0 ) THEN
	       IF( CSPT06(F3,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT06( F3,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF 
            ELSEIF( F2 .GT. 0 ) THEN
	       IF(CSPT05(F2,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT05( F2,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF 
            ELSEIF( F2B .GT. 0 ) THEN
	      IF( CSPT06(F2B,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT06( F2B,V )
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF
            ELSEIF( F1 .GT. 0 ) THEN
	       IF (CSPT03(F1,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT03( F1,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF 
            ELSEIF( F0 .GT. 0 ) THEN
	      IF( CSPT02(F0,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT02( F0,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
              ENDIF 
            ELSEIF( F0B .GT. 0 ) THEN
	      IF( CSPT03(F0B,V) .NE. EMCMISS3 ) THEN
                SPCODE = CSPT03( F0B,V ) 
                CALL SETSOURCE_SMATS
                CYCLE                       !  to end of sources-loop
               ENDIF 
