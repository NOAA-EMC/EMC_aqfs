...
! Get command arguments as strings.
   diag=5
   id_gribdomain=148
   outfile1='aqm.t99z.pm25_24hr_ave.148.grib.bc'
   outfile2='aqm.t99z.pm25_24hr_max.148.grib.bc'
   ave1hr=.true.
   print*,"it is testing"
...
defnum=1             ! Used if igds(3) .ne. 0. The number of entries in array
ideflist
      ideflist=0            ! Used if igds(3) .ne. 0. number of grid points
contained in each row ( or column ), Dummy array otherwise
      call
addgrid(cgrib,max_bytes,igds,igdstmpl,igdstmpllen,ideflist,idefnum,ierr)
