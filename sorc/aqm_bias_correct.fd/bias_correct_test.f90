...
! Local variables.
   ...
   integer ndays, nhours, diag, nsites
   integer cycle_time, start_date, forecast_date, base_year
   integer di, hi, vi, ndays_show,id
...
! Set dynamic run parameters.
   ...
   diag = 4	! Set default verbosity: 0 = errors only, 1 = milestones,
...
!jp      ndays_show = min (ndays, 3)
      ndays_show = ndays
      print *, trim (target_obs_var) // ':    (sample of obs reshaped array)'
!jp      print '(3i6, 5f10.2)', ((di, hi, vi, obs_reshaped(di, hi, vi, 1:5), &
      print '(3i6, 4f10.2)', ((di, hi, vi, obs_reshaped(di, hi, vi, 1:4), &
         hi = 1, 3), di = 1, ndays_show)
      print *
!jp0  write out obs and uncorrected forest at specific grid points
     open(41,file="obs_cmaq_grid1.txt")
     open(42,file="obs_cmaq_grid2.txt")
     do id = 1, nsites
       if ( obs_ids(id) .eq. "000065401"  ) then
         do di =154, ndays_show
!            do hi =1,nhours 
            do hi =1,24
              write(41,199)di,hi,obs_reshaped(di, hi, vi,id) !, &
!                      model_in(di, hi, vi,id)
            enddo
         enddo
       endif  
!
       if ( obs_ids(id) .eq. "000063301"  ) then
         do di =154, ndays_show
!            do hi =1,nhours
            do hi =1,24
              write(42,199)di,hi,obs_reshaped(di, hi, vi,id)  !, &
!                      model_in(di, hi, vi,id)
            enddo
         enddo
       endif
