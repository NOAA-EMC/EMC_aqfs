...
C For Geoff M's smartinit bundle
      PARAMETER(IMAXOT=1500,JJMAXOT=1050,JMAXOT=IMAXOT*JJMAXOT)
      PARAMETER(MAXTILE=54,MAXF=2000,MAXG=10,IBUFSIZE=1200000000)   !!  aqm_Prdgen  May_013
...
C!DIR$ INLINEALWAYS FILTER_SC,FILTER_UV
       LCNTRL=10
       LUGBIN=11
       LUNWGT=20
C
...
cyt print out control file request
      if (NFLDS.le.0) then
       WRITE(6,*)'master control file request wrong ', NFLDS
       stop
      endif
