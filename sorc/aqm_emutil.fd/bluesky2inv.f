...
C                System
C File: @(#)$Id: bluesky2inv.f,v 1.2 2005/07/20 19:08:23 cseppan Exp $
C
...
C smoke@unc.edu
C
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/emutil/bluesky2inv.f,v $
C Last updated: $Date: 2005/07/20 19:08:23 $ 
C
C***********************************************************************
...
C.......  EXTERNAL FUNCTIONS and their descriptions:
     ...
     &              STR2INT, STR2REAL, STRLIST, TRIMLEN

C.......  LOCAL PARAMETERS
      CHARACTER(50), PARAMETER :: CVSW = '$Name: SMOKE_v2_2_11282005 $'  ! CVS release tag
      INTEGER, PARAMETER :: MXSEG = 18    ! number of segments in line
      INTEGER, PARAMETER :: MXOUT = 14    ! number of output values
      REAL,    PARAMETER :: MTON2STON = 1.10231131  ! conversion factor for metric to short tons
      REAL,    PARAMETER :: HR2DAY    = 24.0  !conversion factor for hr to day
      REAL,    PARAMETER :: KG2MTON     = 0.001  !conversion factor for kg to metric tons
...
C...........  Open input file      
          OPEN( FDEV, FILE=LINE(1:nlen), STATUS='OLD', IOSTAT=IOS )
...
C...........  Calculate NH3, SO2, NOX
          ALLVAL( 12 ) = ALLVAL( 7 ) * 0.016                           ! NH3
          ALLVAL( 13 ) = ALLVAL( 7 ) * 0.01867                         ! SO2
          ALLVAL( 14 ) = ALLVAL( 7 ) * 0.05                            ! NOX
         
C...........  Convert values from metric tons to short tons
          DO J = 3, MXOUT 
              ALLVAL( J ) = ALLVAL( J ) * MTON2STON *HR2DAY *KG2MTON
          END DO
