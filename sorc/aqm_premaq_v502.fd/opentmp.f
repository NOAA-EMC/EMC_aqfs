...
C                System
C File: @(#)$Id: opentmp.f,v 1.15 2004/06/21 17:30:55 cseppan Exp $
C
...
C smoke@unc.edu
C
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/temporal/opentmp.f,v $
...
C.............  Double check that pollutant is in the inventory file
     ...
     &                 'I/O API output variables'
        ...
        KNIPPA = K
        ALLOCATE (PVNAMES(KNIPPA))
	ALLOCATE (PVUNITS(KNIPPA))
	PVNAMES(1:KNIPPA) = VNAMESET(1:KNIPPA)
	PVUNITS(1:KNIPPA) = VUNITSET(1:KNIPPA)
!	DO V = 1,KNIPPA
!	write (*,*) PVNAMES(V),PVUNITS(V)
!        ENDDO
C.........  Prompt for and open I/O API output file(s)...
        MESG = 'Enter name for output HOURLY EMISSIONS file'
!AQF        NAMBUF = PROMPTSET( MESG, FSUNKN3, CRL // 'TMP', PROGNAME ) 
        TNAME = NAMBUF

C.........  Open supplemental speciation file
        MESG = 'Enter logical name for the TEMPORAL SUPPLEMENTAL '//
     &         'file'
!AQF        PDEV = PROMPTFFILE( MESG, .FALSE., .TRUE., 
!AQF     &                      CRL // 'TSUP', PROGNAME )
