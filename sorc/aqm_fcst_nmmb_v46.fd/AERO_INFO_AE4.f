...
C $Header: /project/work/rep/CCTM/src/aero/aero4/AERO_INFO_AE4.f,v 1.3 2006/06/12 13:29:11 sjr Exp $

C what(1) key, module and SID; SCCS file; date and time of last delta:
C %W% %P% %G% %U%

C:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
c  MODULE AERO_INFO_AE4 contains what was formerlly in include files
c     AEROSTUFF.EXT AND AERO_internal.EXT as well subroutines used in
c     the fourth version (AE4) of the modal aerosol dynamics model.
c
...
C    Revised 1/06/97 to eliminate most derived constants
...
C                     Kluwer, Dordrecht, 1988
      ...
      MODULE AERO_INFO_AE4
      
      IMPLICIT NONE

C *** set up indices for the array CBLK

      INTEGER VSO4AJ    ! Accumulation mode sulfate aerosol concentration
      INTEGER VSO4AI    ! Aitken mode sulfate concentraton
      INTEGER VNH4AJ    ! Accumulation mode aerosol ammonium concentation
      INTEGER VNH4AI    ! Aitken mode ammonium concentration
      INTEGER VNO3AJ    ! Accumulation mode aerosol nitrate concentration
      INTEGER VNO3AI    ! Aitken mode nitrate concentration
      INTEGER VORGAJ    ! Accumulation mode anthropogenic organic aerosol
                        ! concentration
      INTEGER VORGAI    ! Aitken mode anthropogenic organic aerosol
                        ! concentration
      INTEGER VORGPAJ   ! Accumulation mode primary anthropogenic organic
                        ! aerosol concentration
      INTEGER VORGPAI   ! Aitken mode primary anthropogenic organic
                        ! aerosol concentration
      INTEGER VORGBAJ   ! Accumulation mode biogenic aerosol concentration
      INTEGER VORGBAI   ! Aitken mode biogenic aerosol concentration
      INTEGER VECJ      ! Accumulation mode elemental carbon
      INTEGER VECI      ! Aitken mode elemental carbon
      INTEGER VP25AJ    ! Accumulation mode primary PM2.5 concentration
      INTEGER VP25AI    ! Aitken mode primary PM2.5 concentration
      INTEGER VANTHA    ! coarse mode anthropogenic aerosol concentration
      INTEGER VSOILA    ! coarse mode soil-derived aerosol concentration
      INTEGER VAT0      ! Aitken mode number concentration
      INTEGER VAC0      ! accum  mode number concentration
      INTEGER VCOR0     ! coarse mode number concentration
      INTEGER VSURFAT   ! Index for Aitken mode surface area
      INTEGER VSURFAC   ! Index for accumulatin mode surface area
      INTEGER VAT2      ! Aitken mode mode 2nd moment concentration
      INTEGER VAC2      ! accumulation mode 2nd moment concentration
      INTEGER VH2OAJ    ! Accumulation mode aerosol water concentration
      INTEGER VH2OAI    ! Aitken mode aerosol water concentration
      ...
      INTEGER VAC3      ! accumulation mode 3rd moment concentration
      INTEGER VCOR3     ! coarse mode 3rd moment concentration
      ...
      INTEGER VSGAT     ! Aitken mode diameter
      INTEGER VSGAC     ! accumulation mode diameter
      INTEGER VDGAT     ! Aitken mode standard deviaton
      INTEGER VDGAC     ! accumulation mode standard deviation
      INTEGER VHPLUSJ   ! hydrogen ion concentration in accumulation mode
      INTEGER VHPLUSI   ! hydrogen ion concentration in Aitken mode

c *** indices for the array VAPORS
       
      INTEGER VALKAN    ! condensable alkanes
      INTEGER VALKEN1   ! condensable alkenes - 1
      INTEGER VALKEN2   ! condensable alkenes - 1
      INTEGER VXYLEN1   ! condensable xylenes - 1   
      INTEGER VXYLEN2   ! condensable xylenes - 2
      INTEGER VCRESL    ! condensable cresols
      INTEGER VTOLEN1   ! condensable toluenes - 1
      INTEGER VTOLEN2   ! condensable toluenes - 2
      INTEGER VTERPN1   ! condensable terpenes - 1
      INTEGER VTERPN2   ! condenable terpenes - 2

C *** add diameters to CGRID

      INTEGER DGATKN_DRY ! CGRID pointer to dry Aitken mode diameter
      INTEGER DGACC_DRY  ! CGRID pointer to dry accumulation mode diameter
      INTEGER DGATKN_WET ! CGRID pointer to wet Aitken mode diameter
      INTEGER DGACC_WET  ! CGRID pointer to wet accumulation mode diameter
...
C...Fundamental Constants: ( Source: CRC76, pp 1-1 to 1-6)
      ...
      REAL, PARAMETER :: AVO = 6.0221367 E23
      ...
      REAL, PARAMETER :: STFBLZ = 5.67051 E-8
...
C...mathematical constants:
      ...
      REAL, PARAMETER :: RPI180 = 180.0 / PI

      REAL, PARAMETER :: TWOPI   = 2.0 * PI
      REAL, PARAMETER :: THREEPI = 3.0 * PI
      REAL, PARAMETER :: PI6     = PI / 6.0
      REAL, PARAMETER :: F6DPI   = 6.0 / PI
      REAL, PARAMETER :: F6DPI9  = 1.0 E9 * F6DPI
      REAL, PARAMETER :: F6DPIM9 = 1.0 E-9 * F6DPI
...
C...   to 3rd moment concentration [ m**3 m^-3]
      ...
      REAL, PARAMETER :: SGINICO = 2.2

      ! initial mean diameter for Aitken mode [ m ]
...
C...Variables used in partitioning of volatile inorganics (subroutine TRANSFER)

      ! no. of chemically active aerosol modes 
      INTEGER, PARAMETER :: NMODES = 2         ! currently Aiken and accum only

      ! number of volatile inorganic spcs + H2O
      INTEGER, PARAMETER :: NVOLINORG = 4      ! NH4, Cl, NO3, and H2O

      INTEGER, PARAMETER :: KNH4 = 1  ! index for NH4
      INTEGER, PARAMETER :: KNO3 = 2  ! index for NO3
      INTEGER, PARAMETER :: KCL  = 3  ! index for Cl
      INTEGER, PARAMETER :: KH2O = 4  ! index for H2O

C...Scalar variables for fixed coarse mode standard deviations.

      REAL      EC1       ! exp( log^2( sigmag )/8 )
      REAL      ESC04
      REAL      ESC08
      REAL      ESC12
      REAL      ESC16
...
C *** INTERNAL
      ...
      REAL*8 L2SGAT, L2SGAC   ! square of ln(Sg); used in diameter calcs
      REAL   ESAT36, ESAC36   ! exp(4.5*L2SG); used in diameter calcs
      ...
      REAL*8 MINL2SG ! minimum value of ln(Sg)**2
       PARAMETER( MINL2SG = 2.380480480d-03 ) ! minimum sigma_g = 1.05
      REAL*8 MAXL2SG ! maximum value of ln(Sg)**2
...
C     note: coarse mode has fixed standard deviation, so H2O is added
C     above irrespective of M3_WET_FLAG (assume no SOA in coarse mode)
...
C *** Add water and SOA to third moment if M3_WET_FLAG is .true.
C     note: coarse mode has fixed standard deviation, so H2O is added
C     above irrespective of M3_WET_FLAG; assume no SOA is in coarse mode
...
C *** coarse mode: standard deviation is constant at 2.2

      ESC36 = 1.640346E1 ! exp(4.5*L2SG)
...
c        Dg^3  =  M3 / ( M0 * exp(4.5*ln^2(Sg)) )
     ...
     &           ( CBLK( VAC0 ) * ESAC36 ) ) ** ONE3 )
      ...
      DGCOR  = MAX( DGMIN, (  CBLK( VCOR3) /
     &           ( CBLK( VCOR0) * ESC36) ) ** ONE3  )
...
c      Xst,k equation from Step#5 of Jiang et al. (2005).

      DST25 = 0.5 * ( SQRT( 4 * DCA25 * (DCA25 + B) * 1.0E+03 / RHOP )
     &                    + B * ( B - 1.0 ) )
...
c      in Step#6 of Jiang et al. (2005).
      ...
      DG = DGN * 1.0E+06
      ERFARG = ( LOG(DST25) - LOG(DG) ) / (SQRT2 * XXLSG) - 3. * XXLSG / SQRT2
