...
C $Header: /project/work/rep/CCTM/src/aero/aero4/aero_subs.f,v 1.4 2006/06/02 12:40:47 sjr Exp $
...
c     GETPAR, ORGAER3, EQL3, HCOND3, NEWPART3, GETCOAGS,
...
c     gas-phase species.  See notes in SUBROUTINE EQL3.
      ...
      CALL EQL3( NSPCSDA, CBLK, SO4RATE, GAMMA_N2O5,
...
c      doi:10.1029/2005GL022469
      ...
      SUBROUTINE EQL3( NSPCSDA, CBLK, SO4RATE, GAMMA,
     &                 AIRTEMP, AIRRH, AIRPRS, DT)
...
C *** ARGUMENTS
      ...
      REAL GAMMA             ! N2O5->NO3 reaction probability
      REAL AIRTEMP           ! Air temperature [ K ]
      REAL AIRRH             ! Fractional relative humidity
...
C *** variables for N2O5 -> NO3 conversion
      ...
      REAL      ALPHA     ! parameter needed to estimate GAMMA1
      REAL      BETA      ! parameter needed to estimate GAMMA1
...
c     by Pleim et al (1995) except the reaction probability, GAMMA,
c     is computed using a combination of parameterizations from Riemer 
c     et al (2003) and Evans and Jacob (2005).  The reaction products 
c     are added to the initial HNO3 concentration and used as input 
c     for the thermodynamic equilibrium calculations.

c *** compute GAMMA1 using the sulfate aerosol parameterization given 
c     in Table 1 of Evans and Jacob (2005).  Note that RH in this 
c     subroutine is in fractional units but the parameterization
c     requires RH as a percentage.

      ALPHA = 2.79E-4 + 1.3E-4 * (RH * 100.0) 
     &      - 3.43E-6 * (RH * 100.0) * (RH * 100.0) 
     &      + 7.52E-8 * (RH * 100.0) * (RH * 100.0) * (RH * 100.0)
      
      IF (TEMP .LT. 282.0) BETA  = -0.48                    ! (IF TEMP < 282 K)
      IF (TEMP .GE. 282.0) BETA  = 0.04 * (TEMP - 294.0)    ! (IF TEMP > 282 K)
      
      GAMMA1 = ALPHA * (10.0 ** BETA) 

c *** assume GAMMA1/GAMMA2 = 10, based on Riemer et al (2003).
...
!     KN2O5 =   GAMMA * CBAR * PI * ( XXF_AT + XXF_AC )
      KN2O5 = 0.0   ! switching off heterogeneous pathway
...
c     AERSLD = 0.D0 because of metastable assumption
      ...
      ENDIF
      IF ( TRUSTCL ) THEN
         ACL  = AERLIQ( 4 ) * DFCL  + AERLIQ( 10 ) * DFHCL
         GCL  = GAS( 3 ) * DFHCL
      ENDIF
