...
C                System
C File: @(#)$Id: laypoint.f,v 1.31 2005/10/02 00:18:14 cseppan Exp $
C  
...
C smoke@unc.edu
C  
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/point/laypoint.f,v $
C Last updated: $Date: 2005/10/02 00:18:14 $ 
C  
C***********************************************************************
...
C.............  Allocate per-layer arrays from 0:EMLAYS
            ALLOCATE( PRESF( 0:EMLAYS, NSRC ), STAT=IOS )
...
C.............  Read and transform meteorology:
            ...
            CALL SAFE_READ3(XNAME,'PRES-F_lvl',ALLAYS3,JDATE,JTIME,XBUF)
            CALL BMATVEC( METNGRID, NSRC, EMLAYS+1, NX, CX, XBUF, PRESF)
...
!     &                               ( PRSFC( S ) - 200.) * CONVPA +
!     &                               200. * CONVPA
!                        write (*,*) L, VGLVSXG( L ), PRSFC( S ), PRESF( L )
...
!                            write (*,*)    
!     &                           HFX(S),  TMPBFLX, 
!     &                           DTHDZ(1:EMLAYS),ZTOP, ZBOT, ZPLM
     
!                            write (*,*) S, ZTOP, ZBOT, ZPLM 
...
C.................  Allocate plume to layers
                IF( FIREFLAG ) THEN
                 ...
                 CALL FIRE_POSTPLM( EMLAYS, S, ZBOT, ZTOP, PRESF(0,S), 
     &                                 LFULLHT, TEMPS, LHALFHT, TMPACRE,
     &                                 SFRACT, LTOP, TFRAC )
                ELSE
                    CALL POSTPLM( EMLAYS, S, ZBOT, ZTOP, PRESF(0,S), 
