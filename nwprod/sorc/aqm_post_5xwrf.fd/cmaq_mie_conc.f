      subroutine cmaq_mie_conc(N,M,NFLDS_INT,KPDSIN,KPDSOUT,K5PDS,
     ...
     &          iyear,month,iday,nout,kozone
      real      asigo(22)
      logical   matched,EXT_MIE,EXT_CONC
C----Operate on CMAQ instantaneous conc and vis files
C    aconc file starts at cyc time endfhr files
C    conc file starts at cyc time endfhr+1 files
C    vis  file starts at cyc+1 time endfhr files
C-----------------------------------------------------
      integer   ispec(180:229), nsp_len
      character chform(180:229)*8,spname*8,spcname(200)*16
     ...
     &       "nrec=",nrec
            ...
            do 330 ln = 1,NFLDS_INT
               ...
               if(ln.eq.1) then
                  nD2 = 100+N
                  if(EXT_CONC) nD2 = 100+N-1
                  if(EXT_CONC.and.EXT_MIE) nD2 = 100+N !aconc file
                  write(Dn2,'(i3)') nD2
              ...
              do ik=1,MQK
               ...
               enddo
               if(EXT_MIE.and.EXT_CONC) then
               KPDSOUT(11)=stime*1e-4                   !aconc file
               KPDSOUT(14)=N-1
               KPDSOUT(15)=N
               KPDSOUT(16)=3
               KPDSOUT(17)=3
               else
               KPDSOUT(11)=stime*1e-4                   !conc file
               if(EXT_MIE) KPDSOUT(11)=(stime-100)*1e-4 !vis file
               KPDSOUT(14)=N-1
               if(EXT_MIE) KPDSOUT(14)=N
               KPDSOUT(15)=0
               KPDSOUT(16)=0
               KPDSOUT(17)=0
               endif
