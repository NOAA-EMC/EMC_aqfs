...
C Local Variables:
      ...
      LOGICAL, ALLOCATABLE :: CONC_SPCHIT( : )
      LOGICAL CONC_WRITTEN( NPROCS-1 )
      LOGICAL DONE
      INTEGER ASTAT
...
C (inital data assumed to be in correct output units)
      ...
      IF ( ASTAT .NE. 0 ) THEN
         WRITE( LOGDEV,* ) ' Out of memory allocating CONC_DATA'
         STOP
         END IF

      ALLOCATE ( CONC_SPCHIT( CGRD_NVARS ), STAT=ASTAT )
      IF ( ASTAT .NE. 0 ) THEN
         WRITE( LOGDEV,* ) ' Out of memory allocating CONC_SPCHIT'
         STOP
         END IF
...
!     write( logdev,* ) ' CGRD_DSZE, CONC_GSZE:   ', CGRD_DSZE, CONC_GSZE
     ...
     &                   bounce )
         ...
         IF ( CONC_NPE .GE. NPROCS-1 ) THEN

            IF ( .NOT. WRITE3( CTM_CONC_1, 'ALL', JDATE, JTIME,
     &                         CONC_DATA ) ) THEN
               WRITE( LOGDEV,* ) ' Cannot write file ', CTM_CONC_1
               STOP
               END IF
