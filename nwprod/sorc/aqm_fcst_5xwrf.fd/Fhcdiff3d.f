...
C $Header: /project/work/rep/CCTM/src/hdiff/multiscale/hcdiff3d.F,v 1.5 2002/04/05 18:23:15 yoj Exp $ 
...
C Arguments:
      ...
      REAL         K11BAR( NCOLS+1,NROWS+1,NLAYS ) ! x1-flux points
      REAL         K22BAR( NCOLS+1,NROWS+1,NLAYS ) ! x2-flux points
...
!        WRITE( LOGDEV,* ) ' LVL, EKHMAX = ', LVL, EKHMAX
      ...
      CALL SE_COMM ( EDDYH3D, DSPL_N1_E1_S0_W0, DRCN_N_E )
...
1003  FORMAT( / '@2@Layer', 5X, 'Time Step', 9X, 'EffKB' )
     ...
     &                        K22BAR( COL,ROW,LVL ) )
         ...
         DT = CFC * DX1 * DX2 / SE_GLOBAL_MAX ( EFFKB )
