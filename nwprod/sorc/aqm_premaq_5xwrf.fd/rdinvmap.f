...
C                System
C File: @(#)$Id: rdinvmap.f,v 1.2 2004/06/28 14:23:49 cseppan Exp $
C
...
C smoke@unc.edu
C
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/lib/rdinvmap.f,v $
C Last updated: $Date: 2004/06/28 14:23:49 $ 
C
C***************************************************************************
...
C...........   Other local variables
        ...
        LOGICAL       :: IS_BINARY   ! true if binary ; false otherwise
...
C      for AQF if doing binary version need to check if BINIO=T
       IS_BINARY = ENVYN('BINIO','binary or netcdf',.true.,istat)
       IF (IS_BINARY) THEN

          IF( .NOT. SETENVVAR( 'NOTCDF_FATAL', 'F' ) ) THEN
             EFLAG = .TRUE.
          END IF
...
C............  For /IOAPI/ packet, retrieve physical file name
     ...
     &                     'corrupted for /IOAPI/ packet at line', IREC
                ...
                ELSE
		    IF (IS_BINARY) THEN
		       EPHYS = 'BIN:'// TRIM( PATH ) // LINE( J2+8:L1 )
		    ELSE
...
C........  Open I/O API file
        ELSE 
	    IF( .NOT. OPENSET( ENAME, FSREAD3, PROGNAME ) ) THEN
            EFLAG = .TRUE.
