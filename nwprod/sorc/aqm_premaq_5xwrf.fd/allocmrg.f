...
C         -- expand the dimension of PEMGD to include species dimension so
C            the data can be store in memory rathan than reading from or
C            writing to a file.
C         -- initialize PEMGRD in smkmerge.f instead
C       02/14/05 by David Wong
C         -- removed allocation of LFRAC which is done in pre_point.f
C
C***********************************************************************
C
C Project Title: Sparse Matrix Operator Kernel Emissions (SMOKE) Modeling
C                System
C File: @(#)$Id: allocmrg.f,v 1.17 2004/07/26 14:08:38 cseppan Exp $
C
...
C smoke@unc.edu
C
C Pathname: $Source: /afs/isis/depts/cep/emc/apps/archive/smoke/smoke/src/smkmerge/allocmrg.f,v $
...
C.........  Point source fixed-size arrays
        IF( PFLAG ) THEN
            ...
            ALLOCATE( PEMGRD( NGRID,EMLAYS,SIZE(PEMNAM) ), STAT=IOS ) ! gridded point emissions
...
C           IF( LFLAG ) THEN
C               ALLOCATE( LFRAC( NPSRC,EMLAYS ), STAT=IOS )   ! layer fractions
C               CALL CHECKMEM( IOS, 'LFRAC', PROGNAME )

C           ELSE IF ( EXPLFLAG ) THEN                     ! Explicit plume rise
            IF ( EXPLFLAG ) THEN                     ! Explicit plume rise
...
C               ALLOCATE( LFRAC( NHRSRC,EMLAYS ), STAT=IOS )   ! layer fractions
C               CALL CHECKMEM( IOS, 'LFRAC', PROGNAME )
                ELEVSRC = 0  ! array
