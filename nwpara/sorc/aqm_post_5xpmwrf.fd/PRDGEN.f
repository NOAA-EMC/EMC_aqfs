...
C                         PARAMETER TABLES
C   05-01-18  LEE - ADDED IN CAPABILITY TO HANDLE CMAQ OUTPUTS & GFS
C                   INGEST
C
...
C    
      ...
      PARAMETER(MAXTILE=54,MAXF=2000,MAXG=8,IBUFSIZE=1360000000)
...
!DIR$ INLINEALWAYS FILTER_SC,FILTER_UV
      ...
      do 1000 iloop=1, mloop       !!Loop(((A)))                VVVV
C
...
c        rewind LUNWGT
         ...
         if(.not.notcdf) then    !!Loop(C) PLee, no wgt for notcdf
...
C       INEED=.T. MEANS WE STILL NEED TO UNPACK THIS FIELD
        ...
        DO MM=1,NFILS2                       !!Loop(((F)))
         IF (KGRID(M).EQ.KGRIDA(MM,N)) THEN  !!IF(((G)))
          IF (INEED) THEN                    !!IF((H))
C<<>>PLee: Diagnostics
C       write(6,*)" N=",N,"KGRID_A=",KGRID(M),KGRIDA(MM,N),
C     &  "JPDS5=",JPDS5(N)

           IF (JPDS5(N).EQ.33.OR.JPDS5(N).EQ.196) THEN  !!IF(I)
C
...
C
           IF (JPDS5(N).EQ.33.OR.JPDS5(N).EQ.196) THEN   !!IF((N))
C
...
C<<>>PLee: Diagnostics
        if(JPDS5(N).eq.7.and.JPDS7(N).eq.1060) then
          write(6,*)"a f_sc N=",N,FIN(1:20),(KPDSIN(1:25))
        endif
C
...
C<<>>PLee: Diagnostics
        if(JPDS5(N).eq.7.and.JPDS7(N).eq.1060) then
          write(6,*)"a p_scIN=",N,FIN(1:4),(KPDSIN(1:25))
          write(6,*)"a p_scW=",N11(1:4),N12(1:4),N21(1:4),
     & N22(1:4),WW11(1:4),WW12(1:4),WW21(1:4),WW22(1:4)
          write(6,*)"a p_scOUT=",N,FOUT(1:4)
        endif
...
C
           IF (JPDS5(N).EQ.33.OR.JPDS5(N).EQ.196) THEN  !!IF((O))
...
C
       ...
       ENDIF                                      !!Loop(((E)))
                                                  ...
                                                  !!IF(((D)))
        if(RESEARCH) then
          if(EXT_MIE.or.EXT_CONC) then
           call cmaq_mie_conc(N,M,NFLDS_INT,KPDSIN,KPDSOUT,K5PDS,
     &          MDLID,KGRIDA,MAXG,MAXF,EXT_MIE,EXT_CONC,fnamepre)
          else
     ...
     &           MDLID,KGRIDA,MAXG,MAXF,fnamepre)
       ...
       endif !! PLee, Air Quality               !!IF(((D))) VVVVV 
 30    CONTINUE                                 !!Loop(((30)))
