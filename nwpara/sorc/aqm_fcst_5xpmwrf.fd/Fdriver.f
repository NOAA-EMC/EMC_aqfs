...
C   Sep 07 - Jeff, change NPROCS-1 to globally-known N_WORKERS; clean up
C-----------------------------------------------------------------------
...
C Load the mechanism COMMON ... (next two INCLUDE files)
      ...
      INCLUDE "/meso/save/wx20jy/cctm/BLD_u5a/RXDT.EXT"      ! chemical mechamism reactions DATA
...
!     LOGICAL :: PNEW = .FALSE.
      ...
      character( 40 ) :: str1 = 'Known globally .........................'
      character( 40 ) :: str2 = 'Known locally only .....................'
      integer i
...
C Initialize message-passing groups and communicators
      ...
      LOGDEV = SETUP_LOGDEV ()
      write( logdev,1003 ) str1, G_MYPE, NPROCS, N_WORKERS
      write( logdev,1005 ) WORLD_GROUP, IO_GROUP, WORK_GROUP
      write( logdev,1007 ) str2, IO_MYPE, NPROCS_IO, IO_COMM
      write( logdev,1009 ) WORK_MYPE, NPROCS_W, WORK_COMM
      write( logdev,* ) ' '

1003  format( / 5x, a40 / 5x, 'G_MYPE:', i3, 2x, 'NPROCS:', i3, 2x, 'N_WORKERS:', i3 )
1005  format(   5x, 'WORLD_GROUP:', i3, 2x, 'IO_GROUP:', i3, 2x, 'WORK_GROUP:', i3 )
1007  format( / 5x, a40 / 5x, 'IO_MYPE:', i3, 2x, 'NPROCS_IO:', i3, 2x, 'IO_COMM:', i3 )
1009  format(   5x, 'WORK_MYPE:', i3, 2x, 'NPROCS_W:', i3, 2x, 'WORK_COMM:', i3 )

C Initialize run scenario
      CALL INITSCEN ( STDATE, STTIME, TSTEP, NSTEPS, LOGDEV )
      JDATE = STDATE; JTIME = STTIME

      WORKER = WORK_GROUP_RANK .NE. MPI_UNDEFINED
      WRITER = IO_GROUP_RANK .NE. MPI_UNDEFINED
      ...
      ELSE IF ( WRITER ) THEN
         RANK = IO_MYPE
      END IF
C NOTE: NUMPROCS is the same for both worker and writer
...
C and define vertical layer structure (in module GRID_CONF)
      IF ( .NOT. GRID_INIT ( N_WORKERS, RANK ) ) THEN
...
C Along with stencil exchange, include swap_sandia
      CALL SWAP_INIT_SNL()
...
!    &                        NROWS, NCOLS, NPROW, NPCOL, NUMPROCS,
!        IF ( .NOT. PIO_INIT( LOGDEV, GL_NROWS, GL_NCOLS, NLAYS, NTHIK,
     &                        NROWS, NCOLS, NPROW, NPCOL, NPROCS_W,
     &                        WORK_MYPE ) ) THEN
...
!     write( logdev,* ) 'd - beyond the barrier: ', error
