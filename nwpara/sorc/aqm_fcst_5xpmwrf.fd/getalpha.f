...
C $Header: /project/air5/sjr/CMAS4.5/rel/models/CCTM/src/cloud/cloud_acm/getalpha.f,v 1.1.1.1 2005/09/09 18:56:05 sjr Exp $

C what(1) key, module and SID; SCCS file; date and time of last delta:
C %W% %P% %G% %U%

      SUBROUTINE GETALPHA ( NUMI, MASSI, SURFI, LWC, T, P, RHOAIR,
     &                      ALFA0, ALFA2, ALFA3 )
...
!sjr  REAL, PARAMETER :: AERCONST = 6.0E-9 / ( RHOPART *  PI / 6.0 )
      ...
      REAL, PARAMETER :: RHO_WATER = 1.0E3 ! density of water at 20 C and 1 ATM [kg/m**3]
...
C minimum value of L2SG
      REAL, PARAMETER :: MINL2SG = 2.380480E-3  ! minimum sigma_g = 1.05

C maximum value of L2SG
      REAL, PARAMETER :: MAXL2SG = 8.395887E-1  ! maximum sigma_g = 2.5

C default value of L2SG if no surface area
      REAL, PARAMETER :: DEFL2SG = 2.815663E-1  ! default sigma_g = 1.7
...
C...chech to make sure that there is aerosol mass and number before
C... proceeding, if not set alphas to zero

      IF ( ( NUMI .LE. 0.0 ) .OR. ( MASSI .LE. 0.0 ) .OR.
     &     ( LWC  .LE. 0.0 ) ) THEN
!    &     ( SURFI .LE. 0.0 ) .OR. ( LWC  .LE. 0.0 ) ) THEN
        ALFA0 = 0.0
        ALFA2 = 0.0
        ALFA3 = 0.0
        RETURN
...
C...get aerosol diffusivities
      ...
      DIFF0 = COEF * ( AMM1 + 2.492 * LAMDA * AMM2 ) / AM0
      DIFF2 = COEF * ( AM1  + 2.492 * LAMDA * AM0  ) / AM2
...
!     ALFA0 = COEF4 * DIFF0 * ( 1.0 + 0.5 * CUBRT( PE0 ) )
!     ALFA2 = COEF4 * DIFF2 * ( 1.0 + 0.5 * CUBRT( PE2 ) )
