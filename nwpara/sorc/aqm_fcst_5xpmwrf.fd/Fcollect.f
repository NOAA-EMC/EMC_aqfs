...
!     INCLUDE SUBST_VGRD_ID   ! vertical dimensioning parameters
      ...
      LOGICAL, ALLOCATABLE, SAVE :: CONC_SPCHIT( : ), CONC_WRITTEN( : )
      LOGICAL       :: CONC_DONE
      ...
      LOGICAL, ALLOCATABLE, SAVE :: DDEP_SPCHIT( : ), DDEP_WRITTEN( : )
      LOGICAL       :: DDEP_DONE
      ...
      LOGICAL, ALLOCATABLE, SAVE :: ACONC_SPCHIT( : ), ACONC_WRITTEN( : )
      LOGICAL       :: ACONC_DONE
      ...
      LOGICAL, ALLOCATABLE, SAVE :: WDEP_SPCHIT( : ), WDEP_WRITTEN( : )
      LOGICAL       :: WDEP_DONE
      ...
      LOGICAL, ALLOCATABLE, SAVE :: CLDD_SPCHIT( : ), CLDD_WRITTEN( : )
      LOGICAL       :: CLDD_DONE
      ...
      LOGICAL, ALLOCATABLE, SAVE :: VIS_SPCHIT( : ), VIS_WRITTEN( : )
      LOGICAL       :: VIS_DONE
...
!        end do
         ...
         ALLOCATE ( DDEP_WRITTEN( N_WORKERS ), STAT=ASTAT )
         IF ( ASTAT .NE. 0 ) THEN
            XMSG = 'Error allocating memory for DDEP_WRITTEN'
            CALL M3EXIT ( PNAME, JDATE, JTIME, XMSG, XSTAT1 )
         END IF
...
!        write( logdev,* ) ' ACONC_DSZE, ACONC_GSZE: ', ACONC_DSZE, ACONC_GSZE
         ...
         ALLOCATE ( ACONC_WRITTEN( N_WORKERS ), STAT=ASTAT )
         IF ( ASTAT .NE. 0 ) THEN
            XMSG = 'Error allocating memory for ACONC_WRITTEN'
            CALL M3EXIT ( PNAME, JDATE, JTIME, XMSG, XSTAT1 )
         END IF
...
!        write( logdev,* ) ' WDEP_DSZE, WDEP_GSZE:   ', WDEP_DSZE, WDEP_GSZE
         ...
         ALLOCATE ( WDEP_WRITTEN( N_WORKERS ), STAT=ASTAT )
         IF ( ASTAT .NE. 0 ) THEN
            XMSG = 'Error allocating memory for WDEP_WRITTEN'
            CALL M3EXIT ( PNAME, JDATE, JTIME, XMSG, XSTAT1 )
         END IF
...
!           write( logdev,* ) ' CLDD_DSZE, CLDD_GSZE:   ', CLDD_DSZE, CLDD_GSZE
            ...
            ALLOCATE ( CLDD_WRITTEN( N_WORKERS ), STAT=ASTAT )
            IF ( ASTAT .NE. 0 ) THEN
               XMSG = 'Error allocating memory for CLDD_WRITTEN'
               CALL M3EXIT ( PNAME, JDATE, JTIME, XMSG, XSTAT1 )
            END IF
...
!           write( logdev,* ) ' VIS_DSZE, VIS_GSZE:   ', VIS_DSZE, VIS_GSZE
            ...
            ALLOCATE ( VIS_WRITTEN( N_WORKERS ), STAT=ASTAT )
            IF ( ASTAT .NE. 0 ) THEN
               XMSG = 'Error allocating memory for VIS_WRITTEN'
               CALL M3EXIT ( PNAME, JDATE, JTIME, XMSG, XSTAT1 )
            END IF
...
!           END IF
     ...
     &            'for date and time', JDATE, JTIME
         ...
         IF ( ACONC_NPE .LT. N_WORKERS ) THEN
