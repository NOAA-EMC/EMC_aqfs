...
C $Header: /project/air5/sjr/CMAS4.5/rel/models/CCTM/src/aero/aero3/isofwd.f,v 1.1.1.1 2005/09/09 18:56:04 sjr Exp $ 

C what(1) key, module and SID; SCCS file; date and time of last delta:
C %W% %P% %G% %U%

C This is ISORROPIA Version 1.5 (25 Oct 03).  
C It has been modified in the following ways:
C 1. All DOUBLE PRECISION declarations have been changed to REAL( 8 )
C 2. The subroutine INIT3 has been renamed ISOINIT3 to avoid conflicting
C    with an identically named IO/API function.  
C 3. In numerous places this expression:
C       DD = MAX(BB*BB-4.D0*CC, ZERO)
C    is used in place of:
C       DD = BB*BB-4.D0*CC
C    to ensure that DD is nonnegative prior to computing SQRT(DD).  
C    These patches were proposed by Uma Shankar 12/2001.
C 4. In SUBROUTINE CALCB4, MOLAL(5) is set to be no smaller than TINY.
C
C=======================================================================
C
C *** ISORROPIA CODE
...
C
         BB    = BET + AK1 - GAM
         CC    =-AK1*BET
         DD    = MAX(BB*BB-4.D0*CC, ZERO)       ! US patch 12/20/01
C
C *** SPECIATION & WATER CONTENT ***************************************
C
...
C=======================================================================
C
      REAL( 8 ) FUNCTION FUNCB3A (ZK, Y, X)
      INCLUDE 'isrpia.inc'
      REAL( 8 ) KK
C
C *** SOLVE EQUATIONS ; WITH ITERATIONS FOR ACTIVITY COEF. ************
...
C=======================================================================
C
      REAL( 8 ) FUNCTION FUNCB2B (X,TNH4HS4,TLC)
      INCLUDE 'isrpia.inc'
C
C *** SOLVE EQUATIONS **************************************************
C
...
C=======================================================================
C
      REAL( 8 ) FUNCTION FUNCG4A (X)
      INCLUDE 'isrpia.inc'
C
      REAL( 8 ) LAMDA, NAI, NH4I, NO3I
      COMMON /CASEG/ CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, LAMDA,
     &               PSI1, PSI2, PSI3, PSI4, PSI5, PSI6, PSI7,
     &               A1,   A2,   A3,   A4,   A5,   A6,   A7
C
...
C
      SUBROUTINE CALCH1A
      INCLUDE 'isrpia.inc'
      REAL( 8 ) LAMDA, LAMDA1, LAMDA2, KAPA, KAPA1, KAPA2, NAFR,
     &                 NO3FR
C
C *** CALCULATE NON VOLATILE SOLIDS ***********************************
C
