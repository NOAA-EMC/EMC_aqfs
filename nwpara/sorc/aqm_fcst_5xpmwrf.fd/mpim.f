...
C   Sep 07 - Jeff: change from checking if communicator is undefined to if
C                  rank is undefined; add globally-know N_WORKERS
C-----------------------------------------------------------------------
      ...
      INTEGER IO_GROUP_RANK   ! redundant MYPE in IO group (local)
      INTEGER WORK_GROUP_RANK ! redundant MYPE in worker group (local)
      INTEGER N_WORKERS    ! = NPROCS_W for global
...
C MPI group and communicator setup
      ...
      CALL MPI_GROUP_RANK ( IO_GROUP, IO_GROUP_RANK, ERROR )
      CALL MPI_GROUP_RANK ( WORK_GROUP, WORK_GROUP_RANK, ERROR )

      IO_MYPE = -1
      WORK_MYPE = -1

!     IF ( IO_COMM .NE. MPI_UNDEFINED ) THEN
      IF ( IO_GROUP_RANK .NE. MPI_UNDEFINED ) THEN
         CALL MPI_COMM_RANK ( IO_COMM, IO_MYPE, ERROR )
         CALL MPI_COMM_SIZE ( IO_COMM, NPROCS_IO, ERROR )
         N_WORKERS = NPROCS - NPROCS_IO
!     ELSE IF ( WORK_COMM .NE. MPI_UNDEFINED ) THEN
      ELSE IF ( WORK_GROUP_RANK .NE. MPI_UNDEFINED ) THEN
         CALL MPI_COMM_RANK ( WORK_COMM, WORK_MYPE, ERROR )
         CALL MPI_COMM_SIZE ( WORK_COMM, NPROCS_W, ERROR )
         N_WORKERS = NPROCS_W
      ELSE
         WRITE( *,* ) '*** INVALID MPI COMMUNICATORS'
         STOP
      END IF
