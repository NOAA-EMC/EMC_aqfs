...
if [ "${VERTEXT}" == "Y" ] && [ ! -s ${VERTEXT_COORD_PATH} ]; then
   echo "WARNING :: exaqm_cmaqv531_hi.sh.ecf : VERTEXT is set as ${VERTEXT} and no ${VERTEXT_COORD_PATH} can be found"
fi
...
## Directory defined in JAQM_FORECAST_HI
...
for sectors in ptegu ptnonipm cmv_c1c2 cmv_c3 ; do
...
## Type of point emission diagnosis output is defined as TYPE_POINT_EMIS_OUT ~/jobs/JAQM_FORECAST_HI
## 2DSUM [default] or 3D
ic=0
for sectors in ptegu ptnonipm cmv_c1c2 cmv_c3 ; do
...
## Type of fire emission diagnosis output is defined as TYPE_FIRE_EMIS_OUT ~/jobs/JAQM_FORECAST_HI
## 2DSUM or 3D [default]
...
export OCEAN_1=${FIXaqm}/SSMASK_${RUN}12_80X52.ncf
...
## COMINbc is defined in JAQM_FORECAST_HI for LBC location [same as CMAQ MET location]
export BNDY_GASC_1=${FIXaqm}/${RUN}_80X52_mean_2002${MM}_GEOSCHEM-35L-tracer.nmmb.ncf
...
mpiexec -n $ntasks --cpu-bind core ${EXECaqm}/${EXEC} >> ${pgmout} 2>errfile
export err=$?
