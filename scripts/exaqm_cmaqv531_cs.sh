...
#  8/3/2021  Jianping Huang  clean up in support of WCOSS2 transition
#####################################################################
set -xa
...
export N_EMIS_GR=2
export GR_EMIS_001=${COMINemi}/emis-noRWC-cb6-${YM}.ncf #surface emission file
export GR_EMIS_LAB_001=GRIDDED_EMIS
export GR_EM_SYM_DATE_001=F
export GR_EMIS_002=${COMINemi}/emis-RWC-cb6-${YM}.ncf #surface RWC file
...
## Type of point emission diagnosis output is defined as TYPE_POINT_EMIS_OUT ~/jobs/JAQM_FORECAST_CS
## 2DSUM [default] or 3D
ic=0
for sectors in ptegu ptnonipm pt_oilgas cmv_c1c2_12 cmv_c3_12 othpt ; do
...
## Type of fire emission diagnosis output is defined as TYPE_FIRE_EMIS_OUT ~/jobs/JAQM_FORECAST_CS
## 2DSUM [default] or 3D
...
if [ -s ${FIRE_DIR}/aqm.${FIRE_SUFFIX}.fire_emi_cs_r.ncf ] && [ "${FCST}" == "NO" ]; then
   ...
   echo "WARNING :: ========================================================"
   echo "WARNING :: Can not find ${FIRE_DIR}/aqm.${FIRE_SUFFIX}.fire_emi_cs_r.ncf or ${FIRE_DIR}/aqm.${FIRE_SUFFIX}.fire_emi_cs.ncf"
...
##
## Soft link CONC and ACONC for POST processing in the middle of run
## for 2-stage post processing
##
if [ "${FCST}" == "YES" ]; then
   if [ -e ${COMOUT}/aconc.${cycle}.fcst.status.txt ]; then
      /bin/rm -f ${COMOUT}/aconc.${cycle}.fcst.status.txt
   fi
   ln -s ${DATA}/ACONC_FCST_STATUS      ${COMOUT}/aconc.${cycle}.fcst.status.txt
fi
...
if [ "${SENDCOM}" == "YES" ]  && [ "${FCST}" == "YES" ] && [ -s ${DATA}/SOILOUT.${CTM_APPL} ]; then
   ...
   if [ -s ${DATA}/ACONC_FCST_STATUS ]; then
      mv ${DATA}/ACONC_FCST_STATUS ${COMOUT}/aconc.${cycle}.fcst.status.txt
   fi
