...
#                      .
# Script name:         exaqm_post1_cb6r3_ak.sh.ecf
...
${COPYGB2} -g "${grid198}" -x -i"1 1" tmpfile      aqm.t${cyc}z.grib2_${RUN}ozconnmmb.198
${COPYGB2} -g "${grid198}" -x -i"1 1" tmpfile_pm25 aqm.t${cyc}z.grib2_${RUN}pm25nmmb.198
${COPYGB2} -g "${grid198}" -x -i"1 1" tmpfile.1hr  aqm.t${cyc}z.ave_1hr_o3.198.grib2  
flag_8hr_o3=no
if [ -s tmpfile.8hr ]; then
   flag_8hr_o3=yes
   ${COPYGB2} -g "${grid198}" -x -i"1 1" tmpfile.8hr  aqm.t${cyc}z.ave_8hr_o3.198.grib2  
fi

cp aqm.t${cyc}z.ave_1hr_o3.198.grib2 ${COMOUT}/
if [ "${flag_8hr_o3}" == "yes" ]; then
   cp aqm.t${cyc}z.ave_8hr_o3.198.grib2 ${COMOUT}/
fi
cp aqm.t${cyc}z.grib2_${RUN}pm25nmmb.198 ${COMOUT}/aqm.t${cyc}z.ave_1hr_pm25.198.grib2
...
##############################
   ...
   for hr in 1 8; do
       ...
       ${TOCGRIB2SUPER} < ${PARMaqm}/wmo72/grib2_cmaq_ave_${hr}hr_o3-awpozcon.${cycle}.198
       export err=$?;err_chk
#
...
#########################################
   ## file aqm.t${cyc}z.grib2_${RUN}pm25nmmb.198 is aqm.t${cyc}z.ave_1hr_pm25.198.grib2
   cp ${DATA}/aqm.t${cyc}z.grib2_${RUN}pm25nmmb.198  ${COMOUT}/

   echo 0 > filesize
   export XLFRTEOPTS="unit_vars=yes"
   export FORT11=aqm.t${cyc}z.grib2_${RUN}pm25nmmb.198
   export FORT12="filesize"
   export FORT31=
   export FORT51=grib2.t${cyc}z.awp${RUN}pm25nmmb.temp
   ${TOCGRIB2SUPER} < ${PARMaqm}/wmo72/grib2_aqm_${RUN}pm25nmmb.${cycle}.198
   export err=$?;err_chk
#
   ...
   export FORT12="filesize"
   export FORT31=
   export FORT51=grib2.t${cyc}z.awp${RUN}pm25nmmb.198
   ${TOCGRIB2SUPER} < ${PARMaqm}/wmo72/grib2_aqm_${RUN}pm25nmmb.${cycle}.198
   export err=$?;err_chk
...
##############################
   if [ "${SENDCOM}" == "YES" ]; then
      cp grib2.t${cyc}z.awp${RUN}pm25nmmb.198  ${PCOM}/awpaqm.t${cyc}z.1hpm25.198.grib2
      echo "no files copy"
