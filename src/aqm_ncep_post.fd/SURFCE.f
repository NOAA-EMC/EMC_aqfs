...
! CANOPY CONDUCTANCE
! ONLY OUTPUT NEW LSM FIELDS FOR NMM AND ARW BECAUSE RSM USES OLD SOIL TYPES
! add GFS by jphuang on 5/9/2018
          iSF_SURFACE_PHYSICS=2
      IF (MODELNAME .EQ. 'NCAR'.OR.MODELNAME.EQ.'NMM' .OR. & 
            MODELNAME.EQ.'RAPR' .OR. MODELNAME.EQ.'GFS')THEN
...
!       PRESSURE THICKNESS REQUESTED BY CMAQ
      IF (IGET(283).GT.0) THEN
            ...
            ENDDO
! J.P.HUANG FOR TEST
            PRINT*,"HJP CMAQ HYBRID1_22","PDTOP",PDTOP,"grib=",grib
            ID(1:25) = 0
	    IF(ME == 0)THEN 
	     DO L=1,LM
              print*,'Debug CMAQ 99:',L,PMID(1,1,L),"PDTOP=",PDTOP, &
                 "PT=",PT
	    ...
	    ID(11)=L  
           ...
           elseif(grib=='grib2') then
            ...
            datapd(1:im,1:jend-jsta+1,cfld)=GRID1(1:im,jsta:jend)
            print*,"hjp777,cfld=",cfld,"GRID1=", &
                GRID1((1+im)/2,(jsta+jend)/2)
           endif
      ENDIF
!      
!       SIGMA PRESSURE THICKNESS REQUESTED BY CMAQ
      IF (IGET(273).GT.0) THEN
            ...
            DO I=1,IM
!jp             GRID1(I,J)=PD(I,J)
             GRID1(I,J)=PINT(I,J,LM+1) - PINT(I,J,1) 
            ENDDO
            ENDDO
! J.P.HUANG FOR TEST
          PRINT*,"HJP CMAQ HYBRID1_65","PD",PD(1,JSTA),"grib=",grib
            ID(1:25) = 0
!            IF(ME == 0)THEN
!             DO L=1,LM
...
!jp            fld_info(cfld)%lvl1=L
            fld_info(cfld)%lvl1=1
            fld_info(cfld)%lvl2=LM+1
            datapd(1:im,1:jend-jsta+1,cfld)=GRID1(1:im,jsta:jend)
            print*,"hjp666,fld_info(cfld)%lvl1=",fld_info(cfld)%lvl1, &
                "fld_info(cfld)%lvl2=",fld_info(cfld)%lvl2,&
                "fld_info(cfld)%ifld=",fld_info(cfld)%ifld,&
             "cfld=",cfld,"GRID1=", &
             GRID1((1+im)/2,(jsta+jend)/2)
!
           endif
